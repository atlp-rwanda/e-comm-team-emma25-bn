  ...
  describe('reset password', () => {
    ...
    describe('add token and change password', () => {
      test('incase incorrect token', async () => {
        const response = await supertest(app).patch("/changepassword/josephrukundo2002@gmail.com/65328dba23").send({
      ...
      test('incase of a unmatching passwords', async () => {
        ...
        const response = await supertest(app).patch(`/changepassword/josephrukundo2002@gmail.com/${token.token}`).send({
          newpassword: "newpas",
          confirmpass: "newpaa"
        })
        expect(response.status).toBe(400)
